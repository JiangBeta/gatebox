...
func DeriveRoutes(containers []ProxyableContainer, domains []models.Domain, fragments []models.Fragment, defaultFragmentIDs []string) []models.Service {
...
func deriveSite(c ProxyableContainer, s siteParsed, st derivedState, domains []models.Domain, fragIDByName map[string]string, defaultFragmentIDs []string) *models.Service {
	if s.spec.host == "" {
		return nil
	}
	// 该站点的行级/服务级值
...
func resolveSiteUpstream(c ProxyableContainer, rp, upPort string, idx int, spec siteSpec) (upstream, proto string, passthrough, ok bool, warn string) {
	...
	if upPort != "" {
		if n, err := strconv.Atoi(strings.TrimSpace(upPort)); err == nil && n > 0 && n <= 65535 {
			return fmt.Sprintf("127.0.0.1:%d", n), models.DomainProtoHTTP, false, true, ""
		}
	...
	return fmt.Sprintf("127.0.0.1:%d", hp), models.DomainProtoHTTP, false, true, ""
}
...
func derivedService(c ProxyableContainer, spec siteSpec, domains []models.Domain, upstream []string, upstreamProto string, enabled bool, extras []string, fragRaw string, fragIDByName map[string]string, defaultFragmentIDs []string) models.Service {
	svc := models.Service{
		AppID:           c.Project,
		Description:     c.Labels[LabelDescription],
		Type:            models.RouteTypeReverseProxy,
		Domains:         []models.ProxyDomain{spec.toDomain(domains)},
		HealthURI:       "/",
		Enabled:         enabled,
...
func resolveFragmentIDs(fragRaw string, fragIDByName map[string]string, defaults []string, upstreamProto string) []string {
	...
	if upstreamProto == models.DomainProtoHTTPS {
		set[models.FragmentSkipVerify] = true
	}
...
func (s siteSpec) toDomain(domains []models.Domain) models.ProxyDomain {
	d := models.ProxyDomain{
		Protocol: s.proto,
...
func matchRootDomain(host string, domains []models.Domain) (sub, root string, ok bool) {
	best := ""
...
func withWarn(svc models.Service, warns []string) models.Service {

...
func (s *Store) SaveCredential(c *model.DNSCredential) error {
...
func (s *Store) ListCredentials() ([]model.DNSCredential, error) {
	...
	if out == nil {
		out = []model.DNSCredential{}
	}
	return out, nil
}

// GetCredential 获取单个凭证。
func (s *Store) GetCredential(id string) (*model.DNSCredential, error) {
	var c *model.DNSCredential
	err := s.db.View(func(tx *bolt.Tx) error {
...
func (s *Store) decodeCredential(v []byte) (*model.DNSCredential, error) {
...
func (s *Store) SaveRegistry(r *model.Registry) error {
...
func (s *Store) decodeRegistry(v []byte) (*model.Registry, error) {
	plain, err := s.decrypt(v)
	if err != nil {
		return nil, err
	}
...
func (s *Store) SaveComposeInstance(c *model.ComposeInstance) error {
	plain, err := json.Marshal(c)
	if err != nil {
		return err
	}
...
func (s *Store) ListComposeInstances() ([]model.ComposeInstance, error) {
	var out []model.ComposeInstance
	err := s.db.View(func(tx *bolt.Tx) error {
		return tx.Bucket(bucketCompose).ForEach(func(_, v []byte) error {
			var c model.ComposeInstance
	...
	if out == nil {
		out = []model.ComposeInstance{}
	}
	return out, nil
}

// GetComposeInstance 获取单个编排项目。
func (s *Store) GetComposeInstance(project string) (*model.ComposeInstance, error) {
	var c *model.ComposeInstance
	err := s.db.View(func(tx *bolt.Tx) error {

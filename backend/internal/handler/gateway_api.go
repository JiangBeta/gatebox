...
func (a *gatewayAPI) serviceHealthStatus(svc model.Service) string {
	if svc.Type != model.RouteTypeReverseProxy || len(svc.Upstream) == 0 || a.health == nil {
...
func (in *serviceInput) healthURIValue() string {
	if in.Type != model.RouteTypeReverseProxy {
		return ""
	}
	if in.HealthURI == nil {
		return "/"
...
func (in *serviceInput) toService(appID string, enabled bool, fragIDs []string, createdAt time.Time) model.Service {
	domains := make([]model.ProxyDomain, 0, len(in.Domains))
	for _, d := range in.Domains {
		domains = append(domains, model.ProxyDomain{
...
func (a *gatewayAPI) createApp(w http.ResponseWriter, r *http.Request) {
	...
	app := &model.App{ID: newID(), Name: strings.TrimSpace(in.Name), Description: in.Description, CreatedAt: time.Now()}
	...
	now := time.Now()
	created := make([]model.Service, 0, len(in.Services))
...
func (a *gatewayAPI) listAppServices(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := a.s.GetApp(id); errors.Is(err, repository.ErrNotFound) {
		writeErr(w, http.StatusNotFound, "应用不存在")
		return
	} else if err != nil {
		writeErr(w, http.StatusInternalServerError, err.Error())
...
func (a *gatewayAPI) createOrphanService(w http.ResponseWriter, r *http.Request) {
	...
	if in.AppID != nil && *in.AppID != "" {
		appID = *in.AppID
		if _, err := a.s.GetApp(appID); errors.Is(err, repository.ErrNotFound) {
...
func builtinFragmentByID(id string) (model.FragmentTemplate, bool) {
	for _, b := range caddy.BuiltinFragmentCatalog() {
		if b.ID == id {
			return b, true
		}
	}
	return model.FragmentTemplate{}, false
}

// fragmentToView 构造片段下发视图(内置条目骨架)。
func fragmentToView(f model.Fragment, builtin bool) fragmentView {
...
func (a *gatewayAPI) updateFragment(w http.ResponseWriter, r *http.Request) {
	...
	if b, ok := builtinFragmentByID(id); ok {
		if err := a.s.SaveFragmentToggle(id, model.BuiltinFragmentToggle{
...
func (a *gatewayAPI) deleteVariable(w http.ResponseWriter, r *http.Request) {
	key := r.PathValue("key")
	if _, err := a.s.GetVariable(key); errors.Is(err, repository.ErrNotFound) {
		writeErr(w, http.StatusNotFound, "变量不存在")
		return
	} else if err != nil {
		writeErr(w, http.StatusInternalServerError, err.Error())
...
func (a *gatewayAPI) buildDNSMap() (map[string]model.DNSCredential, error) {
	domains, err := a.s.ListDomains()
	if err != nil {
		return nil, err
	}
	...
	credMap := make(map[string]model.DNSCredential, len(creds))
	for _, c := range creds {
		credMap[c.ID] = c
	}
	dns := make(map[string]model.DNSCredential, len(domains))
...
func (a *gatewayAPI) deriveDockerServices(ctx context.Context) []model.Service {
	...
	for _, b := range caddy.BuiltinFragmentCatalog() {
		fragments = append(fragments, model.Fragment{ID: b.ID, Name: b.Name, Code: b.Code})
	}
...
func proxyableContainers(ctx context.Context, cli *dockerclient.Client, s *repository.Store, includeStopped bool) ([]gateway.ProxyableContainer, error) {

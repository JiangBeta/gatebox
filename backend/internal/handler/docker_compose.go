...
func (d *dockerAPI) listCompose(w http.ResponseWriter, r *http.Request) {
	...
	knownMap := make(map[string]*model.ComposeInstance, len(known))
	for i := range known {
...
func composeViewFromProject(inst *model.ComposeInstance, p compose.Project) composeView {
	source := "external"
	if inst.Managed {
		source = "managed"
	}
...
func (d *dockerAPI) createCompose(w http.ResponseWriter, r *http.Request) {
	...
	if !model.ValidateProjectName(project) {
...
func (d *dockerAPI) loadCompose(w http.ResponseWriter, r *http.Request) (*model.ComposeInstance, bool) {
...
func (d *dockerAPI) composePaths(inst *model.ComposeInstance) (file, dir string) {
	if inst.Managed {

...
func credentialIDSecret(c models.DNSCredential) (id, secret string, ok bool) {
	switch c.Provider {
...
func BuildEntries(services []models.Service, domains []models.Domain, creds []models.DNSCredential) []Entry {
	domBy := make(map[string]models.Domain, len(domains))
	for _, d := range domains {
		domBy[d.Name] = d
	}
	credBy := make(map[string]models.DNSCredential, len(creds))
	for _, c := range creds {
		credBy[c.ID] = c
	}

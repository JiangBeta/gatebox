...
- **设置**：常规（网关端口 / 证书签发 / 关于 / 外观与区域 / 系统信息）+ **变量**（网关与容器统一变量管理，ADR-035）；认证后置（PRD v2 §11，单用户 admin）。
...
设置页采用与其它页一致的**右上 Tab**（`route.meta.tabs` + `?tab=`，`layout.md` §3）：

- **Tab 常规**：承载 2026-09 现状的两张卡片（网关端口、证书签发）与后续规划分区（关于 / 外观与区域 / 系统）。
- **Tab 变量**：网关与容器变量的统一管理入口（ADR-035）。

### 变量（ADR-035）

- **用户变量**：一张可编辑表格（变量名 / 值 / 说明 / 操作），统一存储于 bucket `variables`；网关与容器共用同一份值。键名规则 `[A-Za-z_][A-Za-z0-9_]*`、不得以 `GB_` 开头。
- **引用写法**随上下文而变：网关 `<%KEY%>`、容器 `${KEY}`（compose 原生）——**不统一区隔符**，页面需同时说明两种写法。
- **系统变量**：只读小节，按「网关内置 / 容器内置」分组展示，数据来自 `GET /api/v1/settings/variables/system`（不再前端硬编码）。
- **迁移冲突报告**：两桶合并时的同名异值 / 非法键不自动导入，以可关闭的告警横幅列出，供就地解决；旧 bucket 保留只读备份。
- **已知债务提示**：容器变量为「保存即固化」，改动仅对之后新建 / 重新保存的编排生效（ADR-035 §7），页面须明示。

...
import {
  listContainerVariables, createContainerVariable, updateContainerVariable, deleteContainerVariable,
  type ContainerVariable,
} from '../../api/docker'

const [messageApi, contextHolder] = message.useMessage()
const userVars = ref<ContainerVariable[]>([])
...
const editing = ref<ContainerVariable | null>(null)
const editForm = ref({ value: '', description: '' })

// 系统变量:仅保留 GB_PROJ_FILE(项目默认地址);GB_PROJECT/GB_DISPLAY 已移除
const builtinVars = [
  { key: 'GB_PROJ_FILE', value: '(按项目注入)', description: '项目默认地址：<gatebox>/appData/<项目名>/' },
]
...
async function saveAdd() {
  ...
  try {
    for (const r of rows) {
      await createContainerVariable(r.key, r.value, r.description)
    }
...
async function saveEdit() {
  ...
  try {
    await updateContainerVariable(editing.value.key, editForm.value.value, editForm.value.description)
...
async function doDelete(row: any) {
  if (row._builtin) return
  try {
    await deleteContainerVariable(row.key)
...
async function load() {
  loading.value = true
  try {
    userVars.value = await listContainerVariables()
  } catch (e: any) {
    messageApi.error(e.message)
  } finally {
    loading.value = false
...
<template>
  ...
  <div style="margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center">
    <span style="color: #888; font-size: 13px">用户自定义变量，用于 Docker Compose 中，引用写法 ${KEY};点击可插入到编排 YAML 编辑器</span>
